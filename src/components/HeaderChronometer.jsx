...
const ConditionPill = memo(function ConditionPill({ condition }) {
  ...
  return (
    <div
      title={tooltip}
      className="flex items-center gap-1 border px-1.5 py-0.5 cursor-help"
      style={{ borderColor: `${condition.color}44`, background: `${condition.color}0d` }}
    >
      <div style={{ width: 4, height: 4, borderRadius: "50%", background: condition.color, boxShadow: `0 0 4px ${condition.color}`, flexShrink: 0 }} />
      <span style={{ color: condition.color, fontFamily: "'Orbitron', monospace", fontSize: "7px", letterSpacing: "0.08em", whiteSpace: "nowrap" }}>
        {condition.label}
      </span>
    </div>
  );
...
export default function HeaderChronometer({ animationEnabled, runtimeConfig, appTimezone = "America/Vancouver" }) {
  ...
  return (
    <div className="hidden lg:flex items-center gap-2 ml-2 overflow-hidden">
      {/* Local time */}
      <div className="flex items-center gap-1.5 border px-2 py-1 ds-header-segment" style={{ borderColor: `${C.amber}44` }}>
        <motion.div
          animate={animationEnabled ? { opacity: [1, 0.2, 1] } : { opacity: 1 }}
          transition={animationEnabled ? { duration: 1.2, repeat: Infinity } : undefined}
          style={{ width: 5, height: 5, borderRadius: "50%", background: C.amber, boxShadow: `0 0 5px ${C.amber}`, flexShrink: 0 }}
        />
        <div>
          <div style={{ color: C.textFaint, fontFamily: "'Orbitron', monospace", fontSize: "6.5px", letterSpacing: "0.12em" }}>LOCAL</div>
          <div style={{ color: C.amber, fontFamily: "'Orbitron', monospace", fontSize: "11px", letterSpacing: "0.1em", lineHeight: 1 }}>{localTime}</div>
        </div>
      </div>
      ...
      {/* Game time */}
      <div className="flex items-center gap-1.5 border px-2 py-1 ds-header-segment" style={{ borderColor: `${C.cyan}44` }}>
        <motion.div
          animate={animationEnabled ? { opacity: [1, 0.2, 1] } : { opacity: 1 }}
          transition={animationEnabled ? { duration: 1.2, repeat: Infinity, delay: 0.6 } : undefined}
          style={{ width: 5, height: 5, borderRadius: "50%", background: C.cyan, boxShadow: `0 0 5px ${C.cyan}`, flexShrink: 0 }}
        />
        <div>
          <div style={{ color: C.textFaint, fontFamily: "'Orbitron', monospace", fontSize: "6.5px", letterSpacing: "0.12em" }}>INGAME · D{gameDay}</div>
          <div style={{ color: C.cyan, fontFamily: "'Orbitron', monospace", fontSize: "11px", letterSpacing: "0.1em", lineHeight: 1 }}>
            {formatClock(gameNow, serverTimezone)}
          </div>
        </div>
      </div>
      ...
      {/* Weather telemetry */}
      <div className="flex items-center gap-1 border px-2 py-1 ds-header-segment" style={{ borderColor: `${C.textFaint}22` }}>
        <div style={{ width: 5, height: 5, borderRadius: "50%", background: weather ? C.green : C.red, boxShadow: `0 0 4px ${weather ? C.green : C.red}`, flexShrink: 0 }} />
        <div>
          <div style={{ color: C.textFaint, fontFamily: "'Orbitron', monospace", fontSize: "6.5px", letterSpacing: "0.1em" }}>ENV</div>
          <div style={{ color: C.textDim, fontSize: "9px", letterSpacing: "0.05em", lineHeight: 1 }}>
            {Number.isFinite(weather?.tempF) ? `${Math.round(weather.tempF)}°F` : "--°F"} {Number.isFinite(weather?.windMph) ? `${Math.round(weather.windMph)}mph` : ""}
          </div>
        </div>
      </div>
