...
export default function InAppNotifications() {
  ...
  useEffect(() => {
    const loadRecent = async () => {
      const data = await base44.entities.ServerEvent.filter({ severity: "CRITICAL" }, "-created_date", 5);
      const warns = await base44.entities.ServerEvent.filter({ severity: "ALERT" }, "-created_date", 5);
      const all = [...data, ...warns].sort((a, b) => new Date(b.created_date) - new Date(a.created_date));
      setEvents(all);
      setUnread(all.filter(e => !dismissed.includes(e.id)));
    };
    loadRecent();
